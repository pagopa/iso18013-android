 ...
 */
class DeviceRequestParserRefactor(
     ...
     *
     * @return a [DeviceRequestParserRefactor.DeviceRequest] with the parsed data.
    ...
    class DeviceRequest internal constructor() {
        ...
        private fun checkSignature(
            ...
            message: ByteArray,
            algorithm: AlgorithmRefactor,
            signature: EcSignature
        ): Boolean {
            val signatureAlgorithm = when (algorithm) {
                AlgorithmRefactor.UNSET -> throw IllegalArgumentException("Algorithm not set")
                AlgorithmRefactor.ES256 -> "SHA256withECDSA"
                AlgorithmRefactor.ES384 -> "SHA384withECDSA"
                AlgorithmRefactor.ES512 -> "SHA512withECDSA"
                AlgorithmRefactor.EDDSA -> {
        ...
        private fun coseSign1Check(
            publicKey: EcPublicKey,
            detachedData: ByteArray?,
            signature: CoseSign1,
            signatureAlgorithm: AlgorithmRefactor
        ): Boolean {
            require(
            ...
            request.getOrNull("docRequests")?.let { docRequests ->
                ...
                for (docRequestDataItem in docRequestsDataItems) {
                    ...
                    if (!skipReaderAuthParseAndCheck && readerAuth != null) {
                        ...
                        val signatureAlgorithm = AlgorithmRefactor.fromInt(
                        ...
                        readerAuthenticated = coseSign1Check(
                            readerKey,
