...
internal fun ByteString.extractFromDo53(): ByteString {
    check(this.size >= 2) {
        "DO53 length ${this.size}, expected at least 2"
    }
    with(ByteDataReader(this)) {
        val tag = getUInt8()
...
internal fun ByteString.encapsulateInDo53(): ByteString {
    val data = this
...
internal fun Int.requireInRange(validRange: UIntRange) {
    require(this.toUInt() in validRange) {
        "Int ${
            errRange(
                this,
                validRange.first,
...
internal fun Long.requireInRange(validRange: ULongRange) {
    require(this.toULong() in validRange) {
        "Long ${
            errRange(
                this,
                validRange.first,
...
private fun errPositive(v: Any, type: String) =
